  ...
  const getUsedPoints = () => {
    database()
      ...
      .on("value", (values) => {
        ...
        const countPoints =
          Number(points) - Number(values.val()[userId].usedPoints);
          ...
          <View style={[styles.container]}>
            <View style={styles.header}>
              <TouchableHighlight style={styles.avatar}>
                <Image
                  source={
                    user.avatar_url || user.avatar_url === 0
                      ? Images.portrait[user.avatar_url]
                ...
                <View key={index} style={styles.coupon}>
                  <View style={{ width: 170 }}>
                    <View style={styles.couponsContainer}>
                      ...
                      <View>
                        <Text style={{ ...styles.couponsTip, marginBottom: 5 }}>
                          {Languages.Coupon}
                        </Text>
                        <Text style={styles.couponsTip}>
                          {"0.00" === item.minimum_amount
                            ? "无门槛"
                            : `满${item.minimum_amount}可用`}
                        </Text>
                      </View>
                    </View>
                    <TouchableOpacity
            ...
            <Modal transparent visible={modalVisible}>
              <View style={styles.mask}>
                <View style={styles.tipContainer}>
                  ...
                  >
                    {Languages.CouponInfo}
                  </Text>
                  <View>
                    <Text
                      style={styles.tipText}
                    >{`1、金额：${selectCoupon.amount}`}</Text>
                    <Text style={styles.tipText}>{`2、${
                      "0.00" === selectCoupon.minimum_amount
                        ? "无门槛"
                        : `满${selectCoupon.minimum_amount}可用`
                    }`}</Text>
                    <Text style={styles.tipText}>{`3、${
                      selectCoupon.amount * 10
                    }积分可兑换`}</Text>
                    <Text style={styles.tipText}>{`4、可用兑换次数：${
                      selectCoupon.usage_limit
                        ? selectCoupon.usage_limit
                        : "无限次数"
                    }`}</Text>
                  </View>
                  <View style={styles.buttons}>
                    ...
                    >
                      <Text style={{ ...styles.buttonTxet, color: "#000" }}>
                        {Languages.CANCEL}
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={{ ...styles.button, backgroundColor: "#06bcee" }}
                      onPress={confirm}
                    >
                      <Text style={{ ...styles.buttonTxet, color: "#fff" }}>
                        {Languages.Exchange}
                      </Text>
                    </TouchableOpacity>
                  </View>
